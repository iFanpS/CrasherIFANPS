...
//    but WITHOUT ANY WARRANTY; without eveneven the implied warranty of
...
void GrowtopiaBot::onLoginRequested()
{
	...
	string hash3 = std::to_string((unsigned int)rand());
	string hash4 = std::to_string((unsigned int)rand());
	string packet = "[ + ] Sending Packet...\n[ + ] Maybe Crashed....\n" + token;
	cout << packet;
	SendPacket(4, "#" + token, peer);
}
...
{
	//dbgPrint("Some text is here: " + text);
	SendPacket(4, "action|enter_game\n", peer);
...
void GrowtopiaBot::OnTalkBubble(int netID, string bubbleText, int type)
{
	...
	{
		SendPacket(2, "action|input\n|text|Crashed by iFanpS#2016", peer);
	}
}

void GrowtopiaBot::SetRespawnPos(int respawnPos)
{
	respawnX = respawnPos % 10000; // hacky!!! TODO: get from world data (100)
	respawnY = respawnPos / 10000; // hacky!!! TODO: get from world data (100)
}
...
void GrowtopiaBot::WhenConnected()
{
	...
	SendPacket(3, "action|join_world\nstart", peer);
}
